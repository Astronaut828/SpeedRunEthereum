...
contract DEX {
	...
	function init(uint256 tokens) public payable returns (uint256) {
		require(totalLiquidity == 0, "Already funded");
	 ...
	 * NOTE: xReserves and yReserves are the balance of the assets in the pool.
	 */
	function price(
		uint256 xInput,
		uint256 xReserves,
		...
		uint256 input_amount_with_fee = xInput * 997; // 3% fee using whole uints (997 and 1000)
	...
	function ethToToken() public payable returns (uint256 tokenOutput) {
		require(msg.value > 0, "Can not swap 0 ETH to BAL");

		uint256 ethLp = address(this).balance - msg.value;
		uint256 tokenLp = token.balanceOf(address(this));

		tokenOutput = price(msg.value, ethLp, tokenLp);
	...
	function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
		require(tokenInput > 0, "Can not swap 0 BAL to ETH");

		uint256 ethLp = address(this).balance;
		uint256 tokenLp = token.balanceOf(address(this));

		ethOutput = price(tokenInput, tokenLp, ethLp);
