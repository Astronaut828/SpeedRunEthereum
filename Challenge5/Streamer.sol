...
contract Streamer is Ownable {
    ...
    */
		require(balances[msg.sender] == 0, "Can not open multiple channels");
    ...
    */
		address rube = ecrecover(
			prefixedHashed,
			voucher.sig.v,
			voucher.sig.r,
			voucher.sig.s
		);
		require(balances[rube] > voucher.updatedBalance, "Not enough balance");
		uint256 payment = balances[rube] - voucher.updatedBalance;
		address payable guru = payable(owner());
		guru.transfer(payment);

		balances[rube] = voucher.updatedBalance;

		emit Withdrawn(rube, payment);
	}
